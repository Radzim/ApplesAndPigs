...
def get_eval_table():
    try:
        return pd.read_pickle('eval_table.pkl')
    except:
        ...
        eval_table['successors'] = eval_table.apply(lambda x: successors(x.name) if x['winner']==0 else [], axis=1)
        ...
        eval_table.to_pickle('eval_table.pkl')
        return pd.read_pickle('eval_table.pkl')
